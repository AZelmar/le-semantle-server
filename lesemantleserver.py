...
lexique_base = list(filter(lambda c: (len(c[0]) >= 3 and (c[3] == 'NOM' or c[3] == 'ADJ' or c[3] == 'VER') and
                                    ...
                                    (c[5] == '' or c[5] == 's') and
                                    (c[10] == '' or 'inf' in c[10]) and regex.search(c[0]) == None),
                        csv_reader))
...
# load the model
model_base = KeyedVectors.load_word2vec_format(WORD2VEC_MODEL, binary=True, unicode_errors="ignore")
print(len(model_base))
model = model_base.vectors_for_all(pool_attempt)
print(len(model))
